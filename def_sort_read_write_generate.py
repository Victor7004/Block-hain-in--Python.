...
# Чтение блоков
# Если мы перезапустим программу, то блоки начнут записываться "поверх" старых, соответственно данные будут удалены.
# Чтобы этого не происходило, перед запуском будем считывать блоки из папки.
...
for i in blocks:
 ...
 Также, можно считывать сколько по времени будет длится выполнение программы, для этого дописываем две строчки кода:
